...
```{r}
#Tout ce qui fonctionne pas : 
cox <- coxph(Surv(donor_survival_time, death) ~ fusion_na$`21017240` + frailty(project_code) + project_code:fusion_na$`21017240`, data= fusion_na)
...
X = fusion_na[,signif]
data = fusion_na
res.mca = MCA(X,ncp=2) #5=nb de facteurs par défaut
tmp<-res.mca$ind$coord
colnames(tmp)<-paste0("Factor",1:ncol(tmp))
data<-cbind(data,tmp)
summary(cox<-coxph(formula(paste0("Surv(donor_survival_time, death)~strata(project_code)+",paste(colnames(tmp),collapse="+"))),data=data))
```
