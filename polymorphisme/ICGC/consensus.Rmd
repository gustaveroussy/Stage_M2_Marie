  ...
  error_cohorte = NULL 
    ...
    for (k in 2:ncol(tab)){
      ...
      if(nrow(freq(fusion[,k]))>1 ){
        ...
        #write.table(conting, paste("./Fisher/conting/", colnames(fusion)[k],".txt", sep=""))

        conting_freq <- prop.table(table(fusion[,k], fusion[,which(colnames(fusion)==variable)]), margin=2)
        write.table(conting_freq, paste("./Fisher/conting_freq/", colnames(fusion)[k],".txt", sep=""))
...
write.table(mat_pvalue_cohorte, "./mat_pvalue_project_code_genotype.txt")
...
mat_pvalue <- matrix(NA, ncol = ncol(tab)-1, nrow = 2)
rownames(mat_pvalue) <- c("pvalue", "pvalue_adj")
names=NULL
pvalues=NULL
...
for (i in levels(fusion_random$project_code)){
  ...
  if(length(lignes)>0){
    ...
    conting = NULL
    for (j in 2:length(tab_random)){
      conting <- rbind(conting, table(factor(fusion_random[lignes,j], levels=c("0|0", "0|1", "1|1"))))
    }
    rownames(conting) = colnames(tab_random)[2:length(tab_random)]
    conting <- t(conting)
    write.table(conting, paste("./Fisher/",i,".txt", sep=""))
    test <- try(fisher.test(as.matrix(conting)), silent = TRUE)
