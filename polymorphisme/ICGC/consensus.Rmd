...
help = function(k, data){
  if(is.factor(data[,k])){
    ...
    if(k>=2 && k<=ncol(tab)){ 
      ...
      ggplot(data) + geom_bar(aes(x = data[,k], fill = donor_vital_status)) + xlab(colnames(data)[k]) + theme(legend.position="bottom") + ggtitle(paste(colnames(data)[k], info["REF", k-1], "-->", info["ALT", k-1], sep=" "))
    ...
    hist(data[,k], main=paste(colnames(data)[k], info["REF", k-1], "-->", info["ALT", k-1], sep=" "), xlab=colnames(data)[k])
    dev.off()
  }
}
